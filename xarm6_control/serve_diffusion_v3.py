...
class _ObsBuffer:
    ...
    def push_single(self, obs: Dict):
        ...
        if "robot_state" in obs:
            ...
            j = np.asarray(obs["joint_position"], dtype=np.float32).reshape(-1)
            g = np.atleast_1d(np.asarray(obs["gripper_position"], dtype=np.float32))  # <-- ensure shape (1,)
            state7 = np.concatenate([j, g], axis=0).astype(np.float32)
    ...
    def debug_print(self, prefix: str = "OBSBUF") -> None:
        ...
        for t in range(T):
            ...
            gripper = s[6]
            state_arr = np.concatenate([joints_deg, [gripper]])

            state_str = np.array2string(
                state_arr,
                precision=2,
                floatmode="fixed",
                suppress_small=True,   # ✅ no scientific notation
                max_line_width=200
            )
...
class _DiffusionPolicyAdapter:
    """pi0-like interface:
       - push(obs): update buffer only
       - infer(obs): update buffer and predict -> {'actions': ndarray(T,7)}"""
    ...
    def push(self, obs: Dict) -> bool:
        """Update buffer only; return True if buffer is primed (has T frames)."""
    ...
    async def _handler(self, websocket: websockets.asyncio.server.ServerConnection):
        ...
        while True:
            try:
                ...
                # NEW: client may mark messages as push-only (no inference)
                predict = bool(obs.pop("predict", True))
                ...
                if predict:
                    ...
                    ready = adapter.push(obs)
                    t4 = time.perf_counter()
                    # Lightweight ACK so client knows buffer priming state
                    await websocket.send(packer.pack({"ready": bool(ready)}))
                    t5 = time.perf_counter()
                ...
                if self._perf:
                    print(
                        ...
                        f"{'infer' if predict else 'push '}={(t4-t3)*1e3:.1f}ms "
