...
class ObsBuffer:
    ...
    def push(self, obs: Dict):
        ...
        j = np.asarray(obs["joint_position"], dtype=np.float32)
        g = np.atleast_1d(np.asarray(obs["gripper_position"], dtype=np.float32))
        state7 = np.concatenate([j, g]).astype(np.float32)
...
class RemotePolicyWorker(threading.Thread):
    ...
    def __init__(self,
                 ...
                 H: int,
                 W: int,
                 in_q: "queue.Queue[Tuple[int, Dict, bool]]",
                 out_q: "queue.Queue[Tuple[int, np.ndarray]]",
                 stop_evt: threading.Event,
        ...
        try:
            ...
            async with websockets.connect(self.uri, max_size=None, compression=comp) as ws:
                ...
                while not self.stop_evt.is_set():
                    # Drain to newest job
                    try:
                        seq, obs, predict = self.in_q.get(timeout=0.05)
                        while not self.in_q.empty():
                            seq, obs, predict = self.in_q.get_nowait()
                    except queue.Empty:
                    ...
                    try:
                        base = resize_with_pad(obs["base_rgb"], self.H, self.W)
                        wrist = resize_with_pad(obs["wrist_rgb"], self.H, self.W)
                        gp = np.asarray(obs["gripper_position"], dtype=np.float32).reshape(-1)
                        gp_val = float(gp[0])
                    ...
                    if predict:
                        ...
                        # push-only ack; nothing to queue
                        pass
        except Exception as e:
...
def main(
    ...
    n_obs_steps = 2
    in_q_local: "queue.Queue[Tuple[int, Dict, bool]]" = queue.Queue(maxsize=2)
    ...
    for step_idx in range(max_steps):
        ...
        if not worker_started and (use_remote_policy or (buf and buf.ready())):
            try:
                if use_remote_policy:
                    in_q_local.put_nowait((seq, obs, True))  # request initial chunk
                else:
        ...
        if not out_q.empty():
            try:
                ...
                pass
        # --- Send one message every tick: push-only unless we need to (pre)fetch ---
        if use_remote_policy and worker_started and in_q_local.empty():
            need_infer = False
            if action_chunk is None and next_action_chunk is None:
                # No chunk yet: ask for one
                need_infer = True
            elif next_action_chunk is None:
                # Close to end of current chunk? prefetch
                remaining = action_chunk_len - actions_done
                thresh = max(1, min(prefetch_margin, max(1, action_chunk_len // 2)))
                if remaining <= thresh:
                    need_infer = True
            seq += 1
            try:
                in_q_local.put_nowait((seq, obs, need_infer))
            except queue.Full:
                pass
        
        # Local-mode prefetch (keeps LocalInferenceWorker fed)
        if (not use_remote_policy) and worker_started and in_q_local.empty() and (action_chunk is not None):
            remaining = action_chunk_len - actions_done
            thresh = max(1, min(prefetch_margin, max(1, action_chunk_len // 2)))
            if remaining <= thresh:
                seq += 1
