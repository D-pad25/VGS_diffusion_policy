...
def log_mem(tag: str = ""):
    ...
    if torch.cuda.is_available():
        msg += (f" | GPU alloc={_fmt_bytes(torch.cuda.memory_allocated())}"
...
def approx_replaybuffer_bytes(rb) -> tuple[list[tuple[str, Optional[int]]], int]:
    ...
    total = 0
    rows = []
    for k in ["base_rgb", "wrist_rgb", "robot_state", "action", "episode_ends"]:
        if k in rb:
            arr = rb[k]
            size = getattr(arr, "nbytes", None)
            if size is None:
                try:
                    size = arr[:].nbytes   # may materialize; avoid if huge
                except Exception:
                    size = None
            rows.append((k, size))
            total += (size or 0)
        ...
        if use_cache:
            ...
            with FileLock(cache_lock_path):
                if not os.path.exists(cache_zarr_path):
                    try:
                        if self.mem_debug:
                            log_mem("cache miss → before build_replay_buffer")
                        ...
                        if self.mem_debug:
                            log_mem("after saving cache to disk (before GC)")
                        gc.collect()
                        if torch.cuda.is_available():
                    ...
                    if self.mem_debug:
                        log_mem("cache hit → after load ReplayBuffer into MemoryStore")
